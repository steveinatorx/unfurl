...
const zippedKeys = ['og:image', 'twitter:image', 'twitter:player', 'og:video', 'og:audio'];
...
function unfurl(url, init) {
  ...
  return fetchUrl(url, fetchOpts).then(handleStream(pkgOpts)).then(postProcess(pkgOpts));
}
...
function fetchUrl(url, fetchOpts) {
  ...
  return fetch(url, fetchOpts).then(res => {
    ...
    if (multibyteEncodings.includes(charset)) {
      ...
      res.body = res.body.pipe(iconv.decodeStream(charset)).pipe(iconv.encodeStream('utf-8'));
    }
...
function zipup(target, name, val) {
  ...
  let zipupAs = zippedKeys.map(camelCase).find(key => name.startsWith(key));
...
function postProcess(pkgOpts) {
  return function (pkg) {
    ...
    if (pkgOpts.oembed && pkg.oembed) {
      return fetch(pkg.oembed).then(res => res.json()).then(oembedData => {
        const unwind = get(oembedData, 'body', oembedData, {});

        pkg.oembed = Object.entries(unwind).filter(([key]) => oembed.includes(key)).reduce((obj, [key, value]) => Object.assign(obj, {
          [camelCase(key)]: value
        }), {});
